...
export function ProductCard({ product, index = 0, featured = false }: ProductCardProps) {
  ...
  const bestSeller = index < 3
  const discounted = typeof price === 'number' && price > 0 && product.id.charCodeAt(0) % 5 === 0
      ...
      >
        <div className="relative">
          ...
          <div className="pointer-events-none absolute right-3 top-3 flex flex-col items-start gap-1.5">
            {bestSeller && (
              <span className="rounded-full bg-primary/90 px-2.5 py-1 text-[10px] font-black text-primary-foreground shadow-lg backdrop-blur-md">
                الأكثر مبيعاً
              </span>
            )}
            {discounted && (
              <span className="rounded-full bg-amber-500/90 px-2.5 py-1 text-[10px] font-black text-white shadow-lg backdrop-blur-md">
                خصم خاص
              </span>
            )}
          </div>
