...
export class InnerMonologue {
  ...
  async think(context: any, intent: string): Promise<Thought> {
  ...
  private observe(context: any): string {
  ...
  private async reflect(decision: string, context: any): Promise<{ summary: string; confidence: number }> {
    ...
    if (context.activeErrors?.length > 0) {
      return {
