...
export const submitPrescriptionUpload = createServerFn({ method: 'POST' })
  .inputValidator((raw: unknown) => prescriptionSchema.parse(raw))
  .handler(async ({ data }) => {
...
export const subscribeRefillReminder = createServerFn({ method: 'POST' })
  .inputValidator((raw: unknown) => refillSchema.parse(raw))
  .handler(async ({ data }) => {
    const { supabaseAdmin } = await import('@/integrations/supabase/client.server')
...
export const orderHealthBundle = createServerFn({ method: 'POST' })
  .inputValidator((raw: unknown) => bundleOrderSchema.parse(raw))
  .handler(async ({ data }) => {
    const { supabaseAdmin } = await import('@/integrations/supabase/client.server')
...
export const logWidgetEvent = createServerFn({ method: 'POST' })
  .inputValidator((raw: unknown) => widgetEventSchema.parse(raw))
  .handler(async ({ data }) => {
    try {
      const { supabaseAdmin } = await import('@/integrations/supabase/client.server')
