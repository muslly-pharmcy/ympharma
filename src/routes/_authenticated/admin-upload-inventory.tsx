...
type Status = "idle" | "parsing" | "validated" | "syncing" | "done" | "error";
...
function UploadPage() {
  ...
  async function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!f) return;
    setError(null); setReport(null); setRows([]); setRowErrors([]); setLiveLog([]);
    setStatus("parsing");
    try {
      ...
      json.forEach((r, i) => {
        ...
        if (exp.error) {
          errs.push({ rowNumber, legacyIdRaw: legacyStr, field: "expiry_date", message: exp.error });
        }
        const stock = Math.floor(toNumber(pick(r, ["الرصيــــد", "الرصيد", "Stock", "Qty"])));
        if (stock < 0) {
          errs.push({ rowNumber, legacyIdRaw: legacyStr, field: "stock_qty", message: "الرصيد سالب" });
          return;
        }
        const price = toNumber(pick(r, ["السعر", "القيمــــــة", "القيمة", "Price"]));
        if (price < 0) {
          errs.push({ rowNumber, legacyIdRaw: legacyStr, field: "price", message: "السعر سالب" });
          return;
        }
        valid.push({
          legacyId,
  ...
  async function startSync() {
    ...
    const agg: SyncReport = { total: rows.length, updated: 0, inserted: 0, republished: 0, hidden: 0, errors: [] };
    ...
    <div dir="rtl" className="min-h-screen bg-background p-6">
      <div className="mx-auto max-w-3xl space-y-6">
        <header className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              تحقّق من الصفوف قبل المزامنة، شاهد التقدم لحظيًا، ثم نزّل تقرير CSV.
            </p>
          </div>
        ...
        <div className="rounded-2xl border border-border bg-card p-5 space-y-4">
          ...
          {(status === "syncing" || status === "done") && liveLog.length > 0 && (
        ...
        <div className="rounded-xl border border-dashed border-border bg-card/50 p-4 text-xs text-muted-foreground">
          ...
          <p className="mt-1">يتم التحقق محليًا قبل المزامنة، ومزامنة الصفوف على دفعات من {CHUNK_SIZE} مع شريط تقدم.</p>
        </div>
      </div>
    </div>
  );
