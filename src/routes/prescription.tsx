  ...
  function handleFiles(list: FileList | null) {
    ...
    setFiles((prev) => {
      ...
      for (const file of Array.from(list)) {
        ...
        if (file.size <= 0) { toast.error(`${file.name}: ملف فارغ`); continue; }
        ...
        <form onSubmit={submit} className="space-y-5 rounded-3xl border border-border bg-card p-6 shadow-card">
          <div>
            ...
            >
              ...
              <input ref={inputRef} type="file" accept="image/*,application/pdf,.heic,.heif,.gif,.bmp,.tiff,.webp,.avif,.svg" capture="environment" multiple hidden onChange={(e) => handleFiles(e.target.files)} />
            </div>
