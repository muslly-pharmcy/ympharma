...
echo "==> Mark the synthetic event processed (requires service_role; skipped if denied):"
psql -tAc "UPDATE public.agent_events SET processed_at = now(), processed_by = 'script:test-event-bus' WHERE event_name = 'TestEvent' AND processed_at IS NULL RETURNING id;" 2>/dev/null | head -5 || echo "(skipped — read-only role)"
